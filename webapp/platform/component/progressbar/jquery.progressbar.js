	...
	$.extend({
		progressBar: new function() {
			...
			this.defaults = {
				...
				width			: 121,											// Width of the progressbar - don't forget to adjust your image too!!!												// Image to use in the progressbar. Can be a single image too: 'images/progressbg_green.gif'
				height			: 12,											// Height of the progressbar - don't forget to adjust your image too!!!
				callback		: null,											// Calls back with the config object that has the current percentage, target percentage, current image, etc
				boxImage		: com.ue.global.path + '/platform/component/progressbar/images/progressbar.gif',						// boxImage : image around the progress bar
				barImage		: {
									//0:	CONTEXTPATH + '/common/js/progressbar/images/progressbg_red.gif'
									//30: CONTEXTPATH + '/common/js/progressbar/images/progressbg_orange.gif'
									0: com.ue.global.path + '/platform/component/progressbar/images/progressbg_green.gif'
								},
			...
			this.construct = function(arg1, arg2) {
				...
				return this.each(function(child) {
					...
					if (argvalue != null && this.bar != null && this.config != null) {
						this.config.value 		= parseFloat(argvalue)
						if (argconfig != null)
						...
						config.value			= parseFloat(argvalue);
						...
						for (var i=0; i<numeric.length; i++) 
							config[numeric[i]] = parseFloat(config[numeric[i]]);
					...
					function getText(config) {
						if (config.showText) {
							if (config.textFormat == 'percentage') {
								return " " + Math.round(config.running_value * 100)/100 + "%";
