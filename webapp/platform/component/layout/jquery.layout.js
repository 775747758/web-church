 ...
 */
	...
	,	defaults: { // default options for 'all panes' - will be overridden by 'per-pane settings'
		...
		,	spacing_closed:			7			// ditto - when pane is 'closed'
		,	togglerLength_open:		0			// Length = WIDTH of toggler button on north/south edges - HEIGHT on east/west edges
		...
		,	enableCursorHotkey:		false		// enabled 'cursor' hotkeys.尽量禁止使用快捷键，全局会处理delete、enter事件
 ...
 */
	...
	if (com.ue && com.ue.hp){
		var _webosWin = com.ue.hp.getCurrentWindow();
		if (_webosWin){
			var key = _webosWin.id;
			if (key) {
				var layoutRs;
				if (layoutMap){
					if (layoutMap.containsKey(key)){
						layoutRs = layoutMap.get(key);
					} else {
						layoutRs = new Array();
					}
					layoutRs.push(rs);
