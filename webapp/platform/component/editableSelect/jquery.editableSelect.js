...
(function($) {
	...
	var defaults = {
		...
		textId : "",//edit by wbf
		textName : "",//edit by wbf
		hiddenId : "",//edit by wbf
		hiddenName : "",//edit by wbf

		//rename original select, and call the hidden field the original name attribute?
		renameOriginal : false,//del
	...
	function editableSelect(){
		...
		this.init = function(selectbox,config){
			if (selectbox.tagName.toUpperCase() != "SELECT")
				return;
			config.textId = $(selectbox).attr("textId");
			config.textName = $(selectbox).attr("textName");
			config.hiddenId = $(selectbox).attr("hiddenId");
			config.hiddenName = $(selectbox).attr("hiddenName");
			...
			var inputName = this.selectbox.attr("name") + this.config.suffix;
			var inputId = this.selectbox.attr("id") + this.config.suffix;
			if (this.config.textName) {
				inputName = this.config.textName;
			}
			if (this.config.textId) {
				inputId = this.config.textId;
			}

			this.input = $("<input type='text' />").appendTo(this.wrapper).attr("autocomplete", "off").attr("value", "").attr("name", inputName).attr("id", inputId);
			//把select的样式给input
			var cssClass = this.selectbox.attr("class") ? this.selectbox.attr("class") : "";
			var cssStyle = this.selectbox.attr("style") ? this.selectbox.attr("style") : "";
			this.input.attr("class",cssClass).attr("style",cssStyle).show();
			this.selectbox.attr("class","").attr("style","").hide();

			var origName = this.selectbox.attr("name");
			var newName = origName + this.config.hiddenSuffix;
			var origId = this.selectbox.attr("id");//edit by wbf
			var newId = origId + this.config.hiddenSuffix;//edit by wbf
			var hiddenName = newName;
			var hiddenId = newId;
			if (this.config.hiddenName) {
				hiddenName = this.config.hiddenName;
			}
			if (this.config.hiddenId) {
				hiddenId = this.config.hiddenId;
			}
			//select中选中的value
			this.hidden = $("<input type='hidden' />").appendTo(this.wrapper).attr("autocomplete", "off").attr("value",this.config.initialHiddenValue).attr("name", hiddenName).attr("id", hiddenId);
			...
			this.options.each(function() {
				...
				if (self.config.checkWidth) {
					optWidths.push($("<li />").appendTo(self.list).html("<span>" + optionText + "</span>").addClass("visible").find("span").outerWidth());
				} else {
			...
			//icon定位
			this.icon.css({
				left : this.wrapper.position().left + this.wrapper.outerWidth() - this.icon.width() - 1,
		...
		this.initEvents = function() {
			...
			this.listItems.bind("mousedown", function(e) {
		...
		this.getCurrentTextValue = function() {
		...
		this.getCurrentHiddenValue = function() {
			return this.__getCurrentValue("hidden");
		};

		this.__getValue = function(prop) {
			prop = this[prop];
			if (!this.multiple)
			...
			vals = this.normalizeArray(vals);

			return vals;
		};

		this.__getCurrentValue = function(prop) {
			prop = this[prop];
			if (!this.multiple)
				return $.trim(prop.val());
		...
		this.iconClick = function() {
			if (this.listVisible()) {
				...
				if (this.input.val().length) {
					this.selection(this.input.get(0), 0, this.input
							.val().length);
				}
			}
		};
		...
		this.showList = function() {
			...
			this.listWrapper.removeClass("invisible").addClass(
					"visible");
		...
		this.getListItemsHeight = function() {
		...
		this.highlight = function(activeItem) {
		...
		this.setComboValue = function(val, pop, hideList) {
...
//			this.input.focus();
		...
		this.setHiddenValue = function(val) {
			var set = false;
			val = $.trim(val);
			var oldVal = this.hidden.val();
			if (!this.multiple) {
				for ( var i = 0, len = this.options.length; i < len; ++i) {
					if (val == $.trim(this.options.eq(i).text())) {//edit by wbf
		...
		this.filterFn = function(currentComboValue, itemValue,allComboValues) {
			if ("click" == this.wrapper.data("sc:lastEvent")) {
				return true;
			}
			//alert(currentComboValue.toSource());
			if (!this.multiple) {
				return itemValue.toLowerCase().indexOf(
						currentComboValue.toLowerCase()) == 0;
			} else {
		...
		this.getListMaxHeight = function() {
		...
		this.selection = function(field, start, end) {
		...
		this.normalizeArray = function(arr){
			var result = [];
	...
	$.fn.extend({
		...
		editableSelect:function(method){
			if(!method){
				$(this).each(function(){
					var es = new editableSelect();
					es.init($(this)[0],Array.prototype.slice.call(arguments, 2));
					$(this).data("editableSelect",es);
				});
				return $(this);
			}
			if(method && typeof (method) == 'string' && methods[method]){
				var es = $(this).data("editableSelect");
				return methods[method].apply(es, Array.prototype.slice.call(arguments, 1));
			}
		}
