 ...
 */
package com.unitever.platform.core.util;
...
public class TaskEngine {
	final static Logger logger = LoggerFactory.getLogger(TaskEngine.class);
     ...
     */
    private TaskEngine() {
        timer = new Timer("unitever-timer", true);
        executor = Executors.newCachedThreadPool(new ThreadFactory() {
            ...
            public Thread newThread(Runnable runnable) {
                ...
                Thread thread = new Thread(Thread.currentThread().getThreadGroup(), runnable,
                        "unitever-task-engine-" + threadNumber.getAndIncrement(), 0);
    ...
    public void scheduleAtFixedRate(TimerTask task, long delay, long period) {
        ...
        try {
        	...
        	logger.error("scheduleAtFixedRate error", e);
        }
    }
